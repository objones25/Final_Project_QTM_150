...
**1**
Our final project is on the data set CAB_RIDES: investigating the relationship between time_stamp and price. Later on, we will explore how this relationship is affected by the variables cab_type and name. 
...
```{r}
cab_subset<-mutate(cab_subset, 'time' = anytime(time_stamp/1000) )
cab_subset<-mutate(cab_subset, 'date' = anydate(time_stamp/1000))
ggplot(data = cab_subset)+geom_histogram(mapping = aes(x = time), binwidth = 100, na.rm = T)
```